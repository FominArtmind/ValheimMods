        ...
        public static void AddMagicEffectsPage(TextsDialog textsDialog, Player player)
        {
            ...
            foreach (var entry in magicEffects)
            {
                ...
                foreach (var entry2 in entry.Value)
                {
                    ...
                    t.AppendLine($" <color=silver>- {MagicItem.GetEffectText(effect, item.GetRarity(), magicItem != null ? magicItem.Quality : ItemQuality.Normal, item.m_shared.m_name, false)} ({item.GetDecoratedName()})</color>");
                }
