    ...
    public class AugmentChoiceDialog : MonoBehaviour
    {
        ...
        public void Show(ItemDrop.ItemData fromItem, int effectIndex, Action<ItemDrop.ItemData, int, MagicItemEffect> onCompleteCallback)
        {
            ...
            for (var index = 0; index < effectOptions.Count; index++)
            {
                ...
                text.text = Localization.instance.Localize((index == 0 ? "<color=white>($mod_epicloot_augment_keep)</color> " : "") + MagicItem.GetEffectText(effect, magicItem.GetClass(), rarity, magicItem.Quality, fromItem, true, magicItem.LegendaryID));
